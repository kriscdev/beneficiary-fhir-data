...
# security group and rules to allow nfs to bcda and bfd subnets
#
...
resource "aws_security_group_rule" "bcda" {
  ...
  protocol          = "tcp"
  security_group_id = aws_security_group.bcda_efs_sg.id
  cidr_blocks       = var.bcda_subnet[var.env_config.env]
}

# adds mount targets in each available "data" subnet
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ BCDA EFS ACCESS ROLE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#

# role to allow bcda account mount+rw access (the inline policy allows cross account access)
...
# policy to attach to above role that allows mount+rw access to the fs
...
# attaches above policy to the role
